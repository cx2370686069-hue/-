...
// 这个文件是“支付模块总配置”。
// 支付通道、抽成比例、配送费规则、回调地址、证书路径，都是从这里统一往下发。
/**
 * 支付与分账全局配置
 *
 * 说明：
...
const PAYMENT_CONFIG = {
   ...
   * 4. 商家自配送时，配送收益也归商家
   *
   * 后面如果要正式开启平台抽成，再改这里和结算工具里的规则就行，
   * 这样不用把支付主链整套推翻。
   */
  businessRules: {
    // 一期先不抽成，所以这里先固定为 0。
    commissionRate: 0,
    // 既然平台抽成是 0，这里也先保持 0，避免后面算出脏数据。
    riderShareOfCommission: 0,
    // 县城美食外卖配送计费规则：按腾讯地图驾车路线，15 公里内 25 元，超出部分每公里加 3.5 元
    deliveryPricing: {
      baseDistanceKm: 15,
      baseFee: 25,
      extraPerKm: 3.5
    },
    // 乡镇外卖配送计费规则：3 公里内 2.5 元，3-6 公里按 2.5 元/公里，6 公里后按 6 元/公里
    townDeliveryPricing: {
      baseDistanceKm: 3,
      baseFee: 2.5,
      midDistanceKm: 6,
  ...
  /**
   * 支付宝支付配置（服务端使用）
   * 这里和微信一样，仓库里只保留“从哪读值”，真实密钥必须在服务器环境变量里配。
   */
  alipay: {
