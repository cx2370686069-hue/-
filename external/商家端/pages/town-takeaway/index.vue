<template>
  <view class="container">
    <view class="header">
      <picker mode="selector" :range="townList" :value="townIndex" @change="onTownChange">
        <view class="town-picker">
          <text class="town-label">当前</text>
          <text class="town-text">{{ townList[townIndex] || '请选择乡镇' }}</text>
          <text class="town-switch">切换</text>
    ...
    <view class="category-grid category-grid-4">
      <view
        ...
        v-for="item in townCategoryList"
        :key="item.id"
        @click="onTownCategoryClick(item)"
      >
    ...
    <view class="section" v-if="recommendList.length > 0">
      <view class="section-head">
        <view class="section-head-left">
          <text class="section-title">镇上销量排行榜</text>
        </view>
...
export default {
  ...
  methods: {
    ...
    onTownChange(e) {
      this.townIndex = Number(e.detail.value)
    },
    goDetail(id) {
...
<style scoped>
.container { background-color: #f5f5f5; min-height: 100vh; padding-bottom: 20rpx; }
.header { background: #fff; padding: 20rpx 24rpx 16rpx; box-shadow: 0 2rpx 12rpx rgba(0,0,0,0.06); }
.town-picker { display: flex; align-items: center; background: #f5f5f5; border-radius: 32rpx; padding: 20rpx 24rpx; }
.town-label { font-size: 34rpx; color: #333; margin-right: 16rpx; font-weight: 500; }
.town-text { flex: 1; font-size: 34rpx; color: #333; font-weight: 500; }
.town-switch { font-size: 28rpx; color: #666; margin-right: 8rpx; }
.town-arrow { font-size: 56rpx; color: #999; font-weight: 300; line-height: 1; }
